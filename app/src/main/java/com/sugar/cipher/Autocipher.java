...
public class Autocipher extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_autocipher);
        ...
        SpannableString spannableString = new SpannableString("Autocipher");
        ...
        decrypt = new Decrypt();
        //设置字体
        spannableString.setSpan(new AbsoluteSizeSpan(50), 0, "Autocipher".length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        spannableString.setSpan(new StyleSpan(android.graphics.Typeface.BOLD_ITALIC), 0, "Autocipher".length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        ...
        okbtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (encipherbtn.isChecked()) {//判断用户是否正确输入
                    if (plaintext.equals("")) {
                        Toast.makeText(Autocipher.this, "请输入明文!", Toast.LENGTH_SHORT).show();
                    } else if (keytext.equals("")) {
                        Toast.makeText(Autocipher.this, "请输入秘钥!", Toast.LENGTH_SHORT).show();
                    } else {//输入正确，开始加密：
                        ciphertext = encrypt.Autocipher(plaintext, keytext);
                        cipherEdit.setText(ciphertext);
                    }
                } else if (decipherbtn.isChecked()) {
                    if (ciphertext.equals("")) {
                        Toast.makeText(Autocipher.this, "请输入密文!", Toast.LENGTH_SHORT).show();
                    } else if (keytext.equals("")) {
                        Toast.makeText(Autocipher.this, "请输入秘钥!", Toast.LENGTH_SHORT).show();
                    } else {//输入正确，开始解密:
                        plaintext = decrypt.Autocipher(ciphertext, keytext);
