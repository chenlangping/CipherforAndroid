...
public class DES_socket extends AppCompatActivity {
    ...
    private String showplain = null;//电脑端传来的字符串
    private String flag;
    private MySocketService mySocketService = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_autocipher_socket);
        ...
        subbtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (plainEdit.getText().toString().equals("")) {
                    Toast.makeText(DES_socket.this, "请输入明文！", Toast.LENGTH_SHORT).show();
                } else if (keyEdit.getText().toString().equals("")) {
                    Toast.makeText(DES_socket.this, "请输入秘钥", Toast.LENGTH_SHORT).show();
                } else {
                    if (bindIntent == null) {
                        //如果还没有绑定，则：
                        /*********   绑定socket服务   ***********/
                        bindIntent = new Intent(DES_socket.this, MySocketService.class);
        ...
        decipherbtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (n == i) {
                    Toast.makeText(DES_socket.this, "无消息，请等待...", Toast.LENGTH_SHORT).show();
                } else {
                    if (keyEdit.getText().toString().length() != 7) {
                        Toast.makeText(DES_socket.this, "秘钥长度必须为7！", Toast.LENGTH_SHORT).show();
                    } else {
                        plaintext = decrypt.DES(ciphertext[n], keyEdit.getText().toString());
                        n++;
    ...
    private void sendMessage() throws RemoteException {
        ...
        if (keyEdit.getText().toString().length() != 7) {
            ...
            msg.obj = encrypt.DES(plainEdit.getText().toString(), keyEdit.getText().toString());
            rMessenger.send(msg);
        }
    }
}
