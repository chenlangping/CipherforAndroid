...
public class Permutation extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        encrypt = new Encrypt();
        decrypt = new Decrypt();
        SpannableString spannableString = new SpannableString("Permutation");
        ...
        //设置字体
        spannableString.setSpan(new AbsoluteSizeSpan(50), 0, "Permutation".length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        spannableString.setSpan(new StyleSpan(android.graphics.Typeface.BOLD_ITALIC), 0, "Permutation".length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        ...
        okbtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (encipherbtn.isChecked()) {//判断用户是否正确输入
                    if (plaintext.equals("")) {
                        Toast.makeText(Permutation.this, "请输入明文!", Toast.LENGTH_SHORT).show();
                    } else if (keytext.equals("")) {
                        Toast.makeText(Permutation.this, "请输入秘钥!", Toast.LENGTH_SHORT).show();
                    } else {//输入正确，开始加密：
                        ciphertext = encrypt.Permutationcipher(plaintext, keytext);
                        cipherEdit.setText(ciphertext);
                    }
                } else if (decipherbtn.isChecked()) {
                    if (ciphertext.equals("")) {
                        Toast.makeText(Permutation.this, "请输入密文!", Toast.LENGTH_SHORT).show();
                    } else if (keytext.equals("")) {
                        Toast.makeText(Permutation.this, "请输入秘钥!", Toast.LENGTH_SHORT).show();
                    } else {//输入正确，开始解密:
                        plaintext = decrypt.Permutationcipher(ciphertext, keytext);
